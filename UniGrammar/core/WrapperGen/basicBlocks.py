...
from .primitiveBlocks import AST__slots__, ASTNone, ASTSelf, dirFunc, getAttrFunc, typingIterableAST, typingOptionalAST, typingUnionAST
...
def genFieldClass(name: str, fields: typing.Iterable[str], bases=()) -> ast.ClassDef:
	...
	return ast.ClassDef(name=name, bases=bases, keywords=[], body=[ast.Assign(targets=[AST__slots__], value=ast.Tuple(elts=[ast.Str(f) for f in fields], ctx=ast.Load()), type_comment=None,), gen__init__(fields)], decorator_list=[],)


def genConstructAstObj(varName: str, className: str) -> typing.Tuple[ast.Name, ast.Name, ast.Assign]:
	"""
...
def genAssignStraight(to: ast.Name, fieldName: str, processorFunc: ast.Attribute, o: ast.Name) -> ast.Assign:
	...
	return ast.Assign(
		...
		value=ast.Call(
			func=processorFunc,
			args=[ast.Attribute(value=o, attr=fieldName, ctx=ast.Load())],
			keywords=[],
		),
		type_comment=None
	)
