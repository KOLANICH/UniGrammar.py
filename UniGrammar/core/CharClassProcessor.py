...
from ..utils.charRanges import ranges2CharClassRangedString
...
class CharClassKeepProcessor(CharClassProcessor):  # pylint:disable=abstract-method
	...
	@classmethod
	def union(cls, backend: typing.Type[Generator], union: CharClassUnion, grammar: Grammar) -> str:
		return cls.wrapNegativeOuter(union, cls.selectJoiner(union).join(backend.resolve(c, grammar) for c in union.children))
