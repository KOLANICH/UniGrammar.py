__all__ = (
	"RefParser", "AltCharClassParser", "RangeCharClassParser", "LitCharClassParser", "WellKnownCharClassParser", "AltParser", "LitKeywordsParser", "SeqParser", "OptParser", "TemplateParser",
	"parseRef", "parseAltCharClass", "parseRangeCharClass", "parseLitCharClass", "parseWellKnownCharClass", "parseAlt", "parseLitKeywords", "parseSeq", "parseOpt", "parseTemplateInstantiation",
)
...
from ..core import SectionRecordParser, SectionSubRecordSingleParamParser, IShittyParser
...
class LitCharClassParser(SectionSubRecordSingleParamParser):
	...
	def apply(self, param: str, rec: typing.Mapping[str, typing.Any], recordParser: IShittyParser) -> CharClass:
		...
		if len(param) != 1:
			raise ValueError("`" + self.__class__.ATTR_NAME + "` in `chars` must be exactly 1 char long. For keywords use `keywords` section.", param)
		return self.NODE(param, False)
