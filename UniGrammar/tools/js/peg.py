...
from UniGrammarRuntime.backends.js.peg import PegJSParserFactory, masterBranchURI
...
class PegJSRunner(NotYetImplementedRunner):
	__slots__ = ()

	COMPILER = DummyCompiler
	PARSER = PegJSParserFactory
...
class PegJS(Tool):
	...
	class GENERATOR(SectionedGenerator):
		...
		def Ref(cls, obj, grammar: typing.Optional[Grammar], ctx: typing.Any = None) -> str:
			return transformNameForPegJS(obj.name)

		@classmethod
		def _Name(cls, k: str, v: str, ctx: typing.Any = None) -> str:
			return super()._Name(transformNameForPegJS(k), v, ctx)
